...
def search(domain, limit):
    all_emails = []
    app_email.show_message("[+] Searching in Youtube")
    
    bingUrl = "http://www.bing.com/search?q=site%3Ayoutube.com+%40{word}&count=50&first={counter}"
    app_email.init_search(bingUrl, domain, limit, 0, 50, 'Bing + Youtube')
    app_email.process()
    ...
    app_email.init_search(googleUrl, domain, limit, 0, 100, 'Google + Youtube')
    app_email.process()
    all_emails += app_email.get_emails()
